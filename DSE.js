...
function encrypt(pt, rkb, rk, isDecrypt) {
  if (isDecrypt) return "decryt";
  ...
  const twoDS = [];
  var x = s.length / 8;
  for (var i = 0; i < s.length; i += x / 8) {
    twoDS.push(s.splice(0, x));
  }
  const y = twoDS[0].length / 4;
  const threeDS = twoDS.map((arr) => {
    const temp = arr;
    const temp2 = [];
    for (var i = 0; i < arr.length; i += y / 4) {
      temp2.push(temp.splice(0, y));
    }
    return temp2;
  });
  s = threeDS;
  ...
  // Splitting
  var left = pt.substr(0, 32);
  var right = pt.substr(32, 32);
  ...
  for (let i = 0; i < 16; i++) {
    ...
    var op = "";
    for (let i = 0; i < 8; i++) {
      const getASCIINum = (val) => val.charCodeAt(0) - "0".charCodeAt(0);

      var row = 2 * getASCIINum(x[i * 6]) + getASCIINum(x[i * 6 + 5]);
      // console.log("i ");
      var col =
        8 * getASCIINum(x[i * 6 + 1]) +
        4 * getASCIINum(x[i * 6 + 2]) +
        2 * getASCIINum(x[i * 6 + 3]) +
        getASCIINum(x[i * 6 + 4]);
      var val = s[i][row][col];
      console.log({ i, row, col, val, val: x[i] });
      const getASCII_Char = (val) =>
        String.fromCharCode(val + "0".charCodeAt(0));

      op += getASCII_Char(val / 8);
      val = val % 8;
      op += getASCII_Char(val / 4);
      val = val % 4;
      op += getASCII_Char(val / 2);
      val = val % 2;
      op += getASCII_Char(val);
    }
    // Straight D-box
  ...
  // Final Permutation
  var cipher = bin2hex(permute(combine, final_perm, 64));
  return cipher;
}
...
function main() {
  ...
  /*cout<<"Enter plain text(in hexadecimal): ";
	cin>>pt;
	cout<<"Enter keyHex(in hexadecimal): ";
	cin>>keyHex;*/

  pt = "123456ABCD1325EF";
  keyHex = "AABB09182736CCDD";
  // Key Generation

  // Hex to binary
  var key = hex2bin(keyHex);
  ...
  // Splitting
  var left = key.substr(0, 28);
  var right = key.substr(28, 28);
  ...
  // [9, 18, 22, 25, 35, 38, 43, 54] = missing

  var rkb = []; // rkb for RoundKeys in binary
  var rk = []; // rk for RoundKeys in hexadecimal
  for (var i = 0; i < 16; i++) {
    // Shifting
    ...
    // Key Compression
    var RoundKey = permute(combine, key_comp, 48);
  ...
  // console.log(rkb);
  // console.log("\nEncryption:\n\n");
  var cipher = encrypt(pt, rkb, rk);
  console.log("Encryption Cipher Text: ", cipher);

  var text = encrypt(cipher, rkb.reverse(), rk.reverse(), !true);
  console.log("Decryption Plain Text: ", text);
}
main();
